...
import { useUIStore, useFilterStore, useUndoStore, umaThemes } from './stores';
...
function MainApp() {
  ...
  // Theme-based styles with Uma Musume theme support
  const bgMain = darkMode 
    ? 'bg-gray-900' 
    : isDefaultTheme ? 'bg-gray-50' : '';
  const bgMainStyle = !darkMode && !isDefaultTheme ? { backgroundColor: theme.colors.background } : {};
  
  const bgCard = darkMode ? 'bg-gray-800' : 'bg-white';
  const textPrimary = darkMode 
    ? 'text-white' 
    : isDefaultTheme ? 'text-gray-900' : '';
  const textPrimaryStyle = !darkMode && !isDefaultTheme ? { color: theme.colors.text } : {};
  
  const textSecondary = darkMode 
    ? 'text-gray-400' 
    : isDefaultTheme ? 'text-gray-600' : '';
  const textSecondaryStyle = !darkMode && !isDefaultTheme ? { color: theme.colors.textSecondary } : {};
  
  const borderColor = darkMode 
    ? 'border-gray-700' 
    : isDefaultTheme ? 'border-gray-200' : '';
  const borderColorStyle = !darkMode && !isDefaultTheme ? { borderColor: theme.colors.border } : {};
  
  const inputBg = darkMode ? 'bg-gray-700' : 'bg-white';
  
  // Theme accent color for buttons
  const accentColor = !isDefaultTheme && !darkMode ? theme.colors.primary : '#3B82F6';
  const accentColorSecondary = !isDefaultTheme && !darkMode ? theme.colors.secondary : '#6366F1';

  // Apply theme font
  useEffect(() => {
    if (!isDefaultTheme && !darkMode) {
  ...
  }, [currentTheme, darkMode, theme.fontFamily, isDefaultTheme]);
  ...
  const ItemCard = useCallback(({ item, onEdit }: { item: ItemWithRelations; onEdit: (item: ItemWithRelations) => void }) => {
    ...
    return (
      <div className={`${bgCard} rounded-lg border ${getBorderClass()} overflow-hidden hover:shadow-lg transition-shadow relative`}>
        {/* Item Image */}
        {item.image_url ? (
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => {
    const sidebarBg = !isDefaultTheme && !darkMode 
      ? { backgroundColor: theme.colors.sidebar }
      : {};
    const sidebarTextColor = !isDefaultTheme && !darkMode ? '#fff' : '';
    
    return (
      <div 
        className={`${mobile ? 'w-full' : 'w-56'} ${isDefaultTheme || darkMode ? bgCard : ''} h-full flex flex-col border-r ${isDefaultTheme || darkMode ? borderColor : ''}`}
        style={{ ...sidebarBg, borderColor: !isDefaultTheme && !darkMode ? theme.colors.border : '' }}
      >
        <div className={`p-4 border-b flex items-center justify-between`} style={{ borderColor: !isDefaultTheme && !darkMode ? 'rgba(255,255,255,0.2)' : '' }}>
          <div className="flex items-center gap-2">
            <AppIcon size="sm" />
            <span 
              className={`font-bold text-lg ${isDefaultTheme || darkMode ? textPrimary : ''}`}
        ...
        <div className="p-3 border-t" style={{ borderColor: !isDefaultTheme && !darkMode ? 'rgba(255,255,255,0.2)' : '' }}>
          <div className="grid grid-cols-2 gap-2 mb-3">
            <div 
              className={`p-2 rounded text-center`}
              style={{ backgroundColor: !isDefaultTheme && !darkMode ? 'rgba(255,255,255,0.1)' : (darkMode ? '#374151' : '#F3F4F6') }}
            >
            ...
            <div 
              ...
              style={{ backgroundColor: !isDefaultTheme && !darkMode ? 'rgba(255,255,255,0.1)' : (darkMode ? '#374151' : '#F3F4F6') }}
            >
              <div className="text-lg font-bold text-yellow-400">{items.filter(i => i.status === 'low_stock').length}</div>
  ...
  const SettingsPage = () => {
    ...
    return (
      <div className="space-y-6 max-w-3xl">
        ...
        {/* Theme Selection */}
        <div className={`${bgCard} border ${borderColor} rounded-lg p-4`} style={borderColorStyle}>
          ...
          <div className="mb-4">
            ...
            {/* Color preview */}
            <div className="flex gap-2">
              <div className="flex-1 h-2 rounded" style={{ backgroundColor: theme.colors.primary }} title="Primary" />
              <div className="flex-1 h-2 rounded" style={{ backgroundColor: theme.colors.secondary }} title="Secondary" />
              <div className="flex-1 h-2 rounded" style={{ backgroundColor: theme.colors.accent }} title="Accent" />
              <div className="flex-1 h-2 rounded" style={{ backgroundColor: theme.colors.sidebar }} title="Sidebar" />
            </div>
          </div>

          {showThemePicker && (
            <div className={`border ${borderColor} rounded-lg p-4 mt-4 max-h-96 overflow-y-auto`} style={borderColorStyle}>
                ...
                {umaThemes.map((t) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <div className="font-medium" style={{ color: t.colors.text }}>{t.name}</div>
                        {t.character !== 'None' && (
                          <div className="text-xs" style={{ color: t.colors.textSecondary }}>{t.character}</div>
                        )}
                      </div>
                    </div>
                    <div className="text-xs mb-2" style={{ color: t.colors.textSecondary, fontFamily: t.fontFamily }}>
                      {t.description}
                    </div>
                    <div className="flex gap-1">
                      <div className="w-6 h-3 rounded" style={{ backgroundColor: t.colors.primary }} />
                      <div className="w-6 h-3 rounded" style={{ backgroundColor: t.colors.secondary }} />
                      <div className="w-6 h-3 rounded" style={{ backgroundColor: t.colors.sidebar }} />
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className={`${bgCard} border ${borderColor} rounded-lg p-4`} style={borderColorStyle}>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="relative">
              ...
              {showIconPicker && (
                <div className={`absolute right-0 mt-2 p-3 ${bgCard} border ${borderColor} rounded-lg shadow-lg z-50 w-64`} style={borderColorStyle}>
  ...
  return (
    <div className={`min-h-screen ${bgMain} flex`} style={bgMainStyle}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className={`${bgCard} border-b ${borderColor} px-4 py-3 flex items-center justify-between`} style={borderColorStyle}>
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              {showNotifications && (
                <div className={`absolute right-0 mt-2 w-80 ${bgCard} border ${borderColor} rounded-lg shadow-lg z-50`} style={borderColorStyle}>
